  ...
  class PCO : public webrtc::PeerConnectionObserver {
   ...
   public:
    ...
    void OnDataChannel(rtc::scoped_refptr<webrtc::DataChannelInterface> data_channel) override {
      std::cout << parent.name << ":" << std::this_thread::get_id() << ":"
                << "PeerConnectionObserver::DataChannel(" << data_channel.get() << ", " << parent.data_channel.get() << ")"
                << std::endl;
...
class Wrapper {
 public:
  ...
  void init() {
    ...
    //ice_server.uri = "stun:stun.voipbuster.com:3478";
  ...
  void create_answer_sdp(const std::string &parameter) {
    ...
    connection.peer_connection->SetRemoteDescription(connection.ssdo.get(), session_description);
